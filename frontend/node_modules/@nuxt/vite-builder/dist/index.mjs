import { IS_CSS_RE, isCSS, isVue, parseModuleId, toArray, writeManifest } from "./_chunks/manifest.mjs";
...
function pickSocketPath(platform) {
	const socketName = `nuxt-vite-node-${`${process.pid}-${Date.now()}`}`;
	if (platform === "win32") return { socketPath: join(String.raw`\\.\pipe`, socketName) };
	const parentDir = fs.mkdtempSync(join(os.tmpdir(), "nuxt-vite-node-"));
	fs.chmodSync(parentDir, 448);
	return {
		socketPath: join(parentDir, `${socketName}.sock`),
		parentDir
	};
}
	...
	try {
		server.listen(socketPath, () => {
			try {
				...
				// macOS: chmod on socket may fail — ignore and continue
				console.warn("[nuxt] socket chmod skipped:", error?.message);
			}
		});
	} finally {
