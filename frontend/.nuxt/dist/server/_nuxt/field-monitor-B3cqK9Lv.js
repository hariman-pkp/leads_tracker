import { d as useRuntimeConfig, u as useAuthStore, _ as __nuxt_component_0 } from "../server.mjs";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const filteredTimeline = computed(() => {
      ...
      return (data.value?.timeline || []).filter((v) => v.tgl === selectedTimelineDate.value);
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(activeTab) === "live") {
        ...
        if (unref(liveTeam)) {
          _push(`<span class="text-sm"><span class="text-emerald-400 font-semibold">${ssrInterpolate(unref(liveTeam).active_count)}</span><span class="text-gray-400"> online · </span><span class="text-gray-500 font-semibold">${ssrInterpolate(unref(liveTeam).offline_count)}</span><span class="text-gray-400"> offline</span></span>`);
        } else {
          _push(`<!---->`);
        }
        ...
        _push(`</div><div class="space-y-3"><div class="card"><div class="text-xs font-semibold uppercase tracking-wider text-emerald-400 mb-3 flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse inline-block"></span> Online (${ssrInterpolate(unref(liveTeam)?.active_count || 0)}) </div>`);
        if (!unref(liveTeam)?.active?.length) {
          _push(`<div class="text-sm text-gray-500 text-center py-4"> Belum ada sales aktif </div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<!--[-->`);
        ssrRenderList(unref(liveTeam)?.active || [], (s) => {
        ...
        _push(`<!--]--></div><div class="card"><div class="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-3 flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-gray-600 inline-block"></span> Offline (${ssrInterpolate(unref(liveTeam)?.offline_count || 0)}) </div>`);
        if (!unref(liveTeam)?.offline?.length) {
          _push(`<div class="text-sm text-gray-500 text-center py-3"> — </div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<!--[-->`);
        ssrRenderList(unref(liveTeam)?.offline || [], (s) => {
        ...
        ssrRenderList(unref(liveTrails) || [], (s) => {
          _push(`<div class="flex items-center justify-between text-sm cursor-pointer hover:bg-apex-card/50 rounded px-1 py-1 -mx-1 transition"><div class="flex items-center gap-2"><span class="w-3 h-3 rounded-full inline-block border border-white/20" style="${ssrRenderStyle({ background: unref(salesColorMap)[s.user_id] || "#6366f1" })}"></span><span class="text-gray-300">${ssrInterpolate(s.sales_nama)}</span></div><span class="text-gray-500 text-xs">${ssrInterpolate(s.points?.length || 0)} titik</span></div>`);
        });
        _push(`<!--]-->`);
        if (!unref(liveTrails)?.length) {
      ...
      if (unref(activeTab) === "activity") {
        ...
        ssrRenderList(unref(data)?.sales_list || [], (s) => {
...
//# sourceMappingURL=field-monitor-B3cqK9Lv.js.map
