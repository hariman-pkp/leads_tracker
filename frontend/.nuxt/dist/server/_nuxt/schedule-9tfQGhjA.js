...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(_attrs)}><div class="page-header mb-5"><div><h1 class="page-title"><i class="fa-solid fa-calendar-check text-primary-400 mr-2"></i>FU Schedule</h1><p class="page-subtitle">${ssrInterpolate(unref(data)?.total ?? 0)} lead terjadwal · ${ssrInterpolate(unref(data)?.from)} s/d ${ssrInterpolate(unref(data)?.to)}</p></div><button class="btn-secondary btn-sm"${ssrIncludeBooleanAttr(unref(pending)) ? " disabled" : ""}><i class="${ssrRenderClass(`fa-solid fa-rotate ${unref(pending) ? "fa-spin" : ""}`)}"></i>Refresh </button></div>`);
      if (unref(pending)) {
...
//# sourceMappingURL=schedule-9tfQGhjA.js.map
