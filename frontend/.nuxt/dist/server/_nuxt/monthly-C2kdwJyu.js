...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const detailTotalTarget = computed(
      () => (data.value?.detail_rows ?? []).reduce((s, r) => s + (r.target ?? 0), 0)
    );
    const detailTotalActual = computed(
      () => (data.value?.detail_rows ?? []).reduce((s, r) => s + (r.actual ?? 0), 0)
    );
        ...
        ssrRenderList(unref(data).summary, (row, idx) => {
          ...
          if (unref(selectedMonth) === row.month_num) {
            ...
            if (unref(detailPending)) {
              ...
              _push(`</div></div>`);
              if (unref(data).detail_rows?.length) {
...
//# sourceMappingURL=monthly-C2kdwJyu.js.map
