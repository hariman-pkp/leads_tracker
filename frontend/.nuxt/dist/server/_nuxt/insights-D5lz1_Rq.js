...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const monthlyMax = computed(
      () => Math.max(...(data.value?.monthly_trend ?? []).map((m) => Number(m.jumlah)), 1)
    );
    const velocityMax = computed(
      () => Math.max(...(data.value?.velocity ?? []).map((v) => Number(v.avg_days)), 1)
    );
    const salesMax = computed(
      () => Math.max(...(data.value?.by_sales ?? []).map((s) => Number(s.jumlah)), 1)
    );
    const totalWeighted = computed(
      () => (data.value?.weighted_forecast ?? []).reduce((sum, w) => sum + Number(w.weighted_value), 0)
    );
    const rootCauses = computed(() => {
      ...
      if (onHoldLen > 0) {
        const onHoldVal = (data.value?.onhold_risk ?? []).reduce((sum, l) => sum + Number(l.propose_value), 0);
        causes.push({
          level: "warning",
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pending)) {
        ...
        if (unref(data).monthly_trend?.length) {
        ...
        if (unref(data).source_conversion?.length) {
        ...
        if (unref(data).stale_leads?.length) {
          ...
          ssrRenderList(unref(data).stale_leads, (l) => {
            ...
            _push(`<div class="text-xs text-gray-500">${ssrInterpolate(l.stage)} · ${ssrInterpolate(l.sales_owner || "Unassigned")}</div></div><div class="text-xs text-gray-500 flex-shrink-0">${ssrInterpolate(unref(fmt).rupiah(l.propose_value))}</div></div>`);
          });
          _push(`<!--]--></div>`);
        } else {
        ...
        if (unref(data).weighted_forecast?.length) {
          _push(`<div class="space-y-3"><!--[-->`);
        ...
        _push(`</div><div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5"><div class="card border border-red-900/50"><div class="section-title text-red-400 mb-3"><i class="fa-solid fa-bolt mr-1.5"></i>🔴 Hot Leads Terabaikan </div>`);
        ...
        _push(`</div><div class="card border border-emerald-900/50"><div class="section-title text-emerald-400 mb-3"><i class="fa-solid fa-bullseye mr-1.5"></i>🟢 Siap Closing </div>`);
        if (unref(data).ready_to_close?.length) {
          ...
          ssrRenderList(unref(data).ready_to_close, (l) => {
            ...
            _push(`<div class="text-xs text-gray-500">${ssrInterpolate(l.stage)} · ${ssrInterpolate(l.sales_owner || "—")}</div></div><div class="text-right flex-shrink-0"><div class="text-xs font-bold text-emerald-400">${ssrInterpolate(unref(fmt).rupiah(l.propose_value))}</div><div class="text-xs text-gray-600">${ssrInterpolate(l.probability ?? "—")}%</div></div></div>`);
          });
          _push(`<!--]--></div>`);
        } else {
        ...
        _push(`</div><div class="card border border-orange-900/50"><div class="section-title text-orange-400 mb-3"><i class="fa-solid fa-pause-circle mr-1.5"></i>🟡 On Hold Bernilai Tinggi </div>`);
        if (unref(data).onhold_risk?.length) {
          _push(`<div class="space-y-2 mb-3"><!--[-->`);
        ...
        if (unref(data).by_product?.length) {
...
//# sourceMappingURL=insights-D5lz1_Rq.js.map
