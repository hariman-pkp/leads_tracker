import { _ as __nuxt_component_0, d as useRuntimeConfig } from "../server.mjs";
  ...
  setup(__props) {
    ...
    const statsCards = computed(() => [
      ...
      },
      {
        ...
        value: stats.value?.week_count ?? 0,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(logLoading)) {
        ...
        ssrRenderList(unref(logRows), (r) => {
          ...
          if (r.accuracy_m) {
            _push(`<div class="text-apex-faint mt-0.5">±${ssrInterpolate(r.accuracy_m)}m</div>`);
          } else {
            _push(`<!---->`);
          }
      ...
      if (unref(showCheckinModal)) {
        ...
        if (unref(checkinForm).latitude) {
          ...
          if (unref(checkinForm).accuracy_m) {
            _push(`<span class="text-apex-faint">±${ssrInterpolate(unref(checkinForm).accuracy_m)}m</span>`);
          } else {
            _push(`<!---->`);
          }
...
//# sourceMappingURL=field-activity-xrHj0LAd.js.map
