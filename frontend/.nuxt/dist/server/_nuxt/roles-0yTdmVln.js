        ...
        ssrRenderList(unref(roles2), (role) => {
          ...
          ssrRenderList(unref(menuGroups), (items, group) => {
            ...
            ssrRenderList(items, (m) => {
              _push(`<label class="${ssrRenderClass([unref(roleMenus)[role.id]?.has(m.key) ? "border-primary-600 bg-primary-900/30 text-primary-300" : "border-navy-700 bg-navy-800/50 text-gray-500 hover:border-navy-600", "flex items-center gap-2 cursor-pointer px-3 py-1.5 rounded-lg border text-sm transition-colors"])}" data-v-65ceaf71><input type="checkbox"${ssrIncludeBooleanAttr(unref(roleMenus)[role.id]?.has(m.key)) ? " checked" : ""} class="accent-primary-500" data-v-65ceaf71><i class="${ssrRenderClass(`fa-solid ${m.icon} text-xs`)}" data-v-65ceaf71></i> ${ssrInterpolate(m.label)}</label>`);
            });
            _push(`<!--]--></div></div>`);
          });
          _push(`<!--]--></div>`);
...
//# sourceMappingURL=roles-0yTdmVln.js.map
