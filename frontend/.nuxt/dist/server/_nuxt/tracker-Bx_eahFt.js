...
import { _ as _sfc_main$2 } from "./ProductSelect-DXo4uezQ.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    function terminSlice(projectId) {
      ...
      const page2 = terminPages.value[projectId] ?? 1;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(_attrs)}><div class="page-header mb-5"><div><h1 class="page-title"><i class="fa-solid fa-tasks text-primary-400 mr-2"></i>Revenue Tracker</h1><p class="page-subtitle">${ssrInterpolate(unref(data)?.total_projects)} proyek · ${ssrInterpolate(unref(selectedYear))}</p></div><div class="flex gap-2 flex-wrap">`);
      ...
      if (unref(data)) {
        ...
        _push(`</div>`);
        if (unref(data).unpaid_invoices?.length) {
          _push(`<div class="space-y-2"><!--[-->`);
          ssrRenderList(unref(data).unpaid_invoices, (inv2) => {
            _push(`<div class="flex items-center gap-2 p-2 rounded-lg bg-orange-900/10 border border-orange-900/20 hover:border-orange-700/40 transition-colors"><div class="flex-1 min-w-0"><div class="text-xs font-medium text-gray-200 truncate">${ssrInterpolate(inv2.client)}</div><div class="text-xs text-gray-500">${ssrInterpolate(inv2.invoice_no || inv2.project_id)} · ${ssrInterpolate(inv2.period)}</div></div><div class="text-right flex-shrink-0"><div class="text-xs font-semibold text-orange-400">${ssrInterpolate(unref(fmt).rupiah(inv2.outstanding))}</div><div class="text-xs text-gray-600">${ssrInterpolate(unref(fmt).tgl(inv2.invoice_date))}</div></div></div>`);
          });
          _push(`<!--]--></div>`);
        } else {
      ...
      if (unref(pending)) {
        ...
        ssrRenderList(unref(data)?.projects, (p) => {
            ...
            if (unref(monthlyLoading).has(p.project_id)) {
              ...
              if (terminTotalPages(p.project_id) > 1) {
                ...
                _push(ssrRenderComponent(_component_AppPagination, {
                  page: unref(terminPages)[p.project_id] ?? 1,
...
//# sourceMappingURL=tracker-Bx_eahFt.js.map
