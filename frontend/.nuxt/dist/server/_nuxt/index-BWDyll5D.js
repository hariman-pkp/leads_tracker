...
import { defineComponent, useSSRContext, withAsyncContext, ref, reactive, watch, withCtx, createVNode, unref, createTextVNode, toDisplayString, openBlock, createBlock } from "vue";
import { ssrRenderAttrs, ssrInterpolate, ssrRenderSlot, ssrRenderComponent, ssrRenderClass, ssrRenderList, ssrRenderAttr, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual } from "vue/server-renderer";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div><h1 class="page-title">${ssrInterpolate(unref(data)?.lead?.nama_company)}</h1><p class="page-subtitle">${ssrInterpolate(unref(data)?.lead?.lead_id)} · ${ssrInterpolate(unref(data)?.lead?.product)}</p></div><div class="ml-auto flex gap-2">`);
      _push(ssrRenderComponent(_component_NuxtLink, {
      ...
      if (unref(pending)) {
        ...
        _push(`</div><div class="card"><div class="flex items-center justify-between mb-3"><div class="section-title mb-0"><i class="fa-solid fa-phone-alt mr-1"></i>Follow-Up Log (${ssrInterpolate(unref(data).fu_logs?.length || 0)})</div><button class="btn-secondary btn-sm"><i class="fa-solid fa-plus"></i>Catat FU </button></div>`);
        if (unref(showFuForm)) {
        ...
        if (unref(showHistory)) {
          ...
          if (unref(historyLoading)) {
            ...
            ssrRenderList(unref(history), (h) => {
              _push(`<div class="py-2.5 flex gap-3 items-start text-xs"><div class="w-1.5 h-1.5 rounded-full bg-primary-600 mt-1.5 flex-shrink-0"></div><div class="flex-1 min-w-0"><span class="text-gray-400">${ssrInterpolate(h.field_name)}</span><span class="mx-1.5 text-gray-600">·</span>`);
              if (h.old_value) {
        ...
        _push(`</div></div><div class="card"><div class="section-title"><i class="fa-solid fa-address-book mr-1"></i>Contacts (${ssrInterpolate(unref(data).contacts?.length || 0)})</div>`);
...
//# sourceMappingURL=index-BWDyll5D.js.map
