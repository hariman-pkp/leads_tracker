    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        ssrRenderList(unref(rows), (row) => {
          ...
          if (row.ytd_target > 0) {
            _push(`<span> / ${ssrInterpolate(unref(fmt).rupiah(row.ytd_target))} <span class="${ssrRenderClass([achColor(row.ytd_achievement_pct), "font-semibold ml-1"])}"> (${ssrInterpolate((_a = row.ytd_achievement_pct) != null ? _a : "\u2014")}%) </span></span>`);
          } else {
            _push(`<span class="text-apex-muted ml-1">(target belum diset)</span>`);
          }
          _push(`</p></div></div>`);
          if (row.ytd_target > 0) {
            _push(`<div class="w-32 hidden md:block"><div class="h-2 bg-apex-border/30 rounded-full overflow-hidden"><div class="${ssrRenderClass([achBg(row.ytd_achievement_pct), "h-2 rounded-full transition-all"])}" style="${ssrRenderStyle({ width: Math.min(100, (_b = row.ytd_achievement_pct) != null ? _b : 0) + "%" })}"></div></div></div>`);
          } else {
            _push(`<!---->`);
          }
...
//# sourceMappingURL=sales-target-qGVpKI8_.mjs.map
