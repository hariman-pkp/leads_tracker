...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const totalProjects = computed(
      () => {
        var _a, _b;
        return ((_b = (_a = data.value) == null ? void 0 : _a.by_status) != null ? _b : []).reduce((s, r) => s + r.cnt, 0);
      }
    );
    const zeroValue = computed(
      () => {
        var _a, _b;
        return ((_b = (_a = data.value) == null ? void 0 : _a.zero_projects) != null ? _b : []).reduce((s, p) => s + Number(p.revenue_target), 0);
      }
    );
    const criticalValue = computed(
      () => {
        var _a, _b;
        return ((_b = (_a = data.value) == null ? void 0 : _a.at_risk_projects) != null ? _b : []).reduce((s, p) => s + Number(p.revenue_target), 0);
      }
    );
    const atRiskPage = ref(1);
    const atRiskTotal = computed(() => {
      var _a, _b, _c;
      return (_c = (_b = (_a = data.value) == null ? void 0 : _a.at_risk_projects) == null ? void 0 : _b.length) != null ? _c : 0;
    });
    const atRiskPages = computed(() => Math.ceil(atRiskTotal.value / INSIGHT_PER_PAGE) || 1);
    const atRiskSlice = computed(() => {
      var _a, _b;
      const all = (_b = (_a = data.value) == null ? void 0 : _a.at_risk_projects) != null ? _b : [];
      const start = (atRiskPage.value - 1) * INSIGHT_PER_PAGE;
      return all.slice(start, start + INSIGHT_PER_PAGE);
    });
    const zeroPage = ref(1);
    const zeroTotal = computed(() => {
      var _a, _b, _c;
      return (_c = (_b = (_a = data.value) == null ? void 0 : _a.zero_projects) == null ? void 0 : _b.length) != null ? _c : 0;
    });
    const zeroPages = computed(() => Math.ceil(zeroTotal.value / INSIGHT_PER_PAGE) || 1);
    const zeroSlice = computed(() => {
      var _a, _b;
      const all = (_b = (_a = data.value) == null ? void 0 : _a.zero_projects) != null ? _b : [];
      const start = (zeroPage.value - 1) * INSIGHT_PER_PAGE;
      return all.slice(start, start + INSIGHT_PER_PAGE);
    });
    const ytdActual = computed(() => {
      var _a, _b, _c, _d;
      const m = (_b = (_a = data.value) == null ? void 0 : _a.monthly) != null ? _b : [];
      const cur = (_d = (_c = data.value) == null ? void 0 : _c.cur_month) != null ? _d : 0;
    ...
    const monthlyMax = computed(
      () => {
        var _a, _b;
        return Math.max(...((_b = (_a = data.value) == null ? void 0 : _a.monthly) != null ? _b : []).map((m) => Math.max(Number(m.target), Number(m.actual))), 1);
      }
    );
    const typeMax = computed(
      () => {
        var _a, _b;
        return Math.max(...((_b = (_a = data.value) == null ? void 0 : _a.by_type) != null ? _b : []).map((t) => Number(t.actual)), 1);
      }
    );
    const riskMax = computed(
      () => {
        var _a, _b;
        return Math.max(...((_b = (_a = data.value) == null ? void 0 : _a.by_risk) != null ? _b : []).map((r) => Number(r.target)), 1);
      }
    );
    const kategoriItems = computed(() => {
      var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l;
      return [
        { label: "Project", ach: (_b = (_a = data.value) == null ? void 0 : _a.project_ach) != null ? _b : 0, actual: (_d = (_c = data.value) == null ? void 0 : _c.project_actual) != null ? _d : 0, target: (_f = (_e = data.value) == null ? void 0 : _e.project_target) != null ? _f : 0 },
        { label: "Recurring", ach: (_h = (_g = data.value) == null ? void 0 : _g.recurring_ach) != null ? _h : 0, actual: (_j = (_i = data.value) == null ? void 0 : _i.recurring_actual) != null ? _j : 0, target: (_l = (_k = data.value) == null ? void 0 : _k.recurring_target) != null ? _l : 0 }
      ];
    });
    const ownerItems = computed(() => {
      var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l;
      return [
        { label: "FSP-ECO", ach: (_b = (_a = data.value) == null ? void 0 : _a.fsp_eco_ach) != null ? _b : 0, actual: (_d = (_c = data.value) == null ? void 0 : _c.fsp_eco_actual) != null ? _d : 0, target: (_f = (_e = data.value) == null ? void 0 : _e.fsp_eco_target) != null ? _f : 0 },
        { label: "FSP-CORE", ach: (_h = (_g = data.value) == null ? void 0 : _g.fsp_core_ach) != null ? _h : 0, actual: (_j = (_i = data.value) == null ? void 0 : _i.fsp_core_actual) != null ? _j : 0, target: (_l = (_k = data.value) == null ? void 0 : _k.fsp_core_target) != null ? _l : 0 }
      ];
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if ((_a = unref(data)) == null ? void 0 : _a.years) {
      ...
      if (unref(pending)) {
        ...
        ssrRenderList(unref(data).top_contributors, (p, i) => {
          _push(`<div class="flex items-center gap-3 p-2.5 rounded-lg bg-navy-800/40"><div class="${ssrRenderClass([i === 0 ? "bg-yellow-900/60 text-yellow-400" : i === 1 ? "bg-gray-700 text-gray-300" : "bg-navy-700 text-gray-500", "w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"])}">${ssrInterpolate(i + 1)}</div><div class="flex-1 min-w-0"><div class="text-sm font-medium text-gray-200 truncate">${ssrInterpolate(p.client)}</div><div class="text-xs text-gray-500">${ssrInterpolate(p.project_id)} \xB7 ${ssrInterpolate(p.product)} \xB7 ${ssrInterpolate(p.organisasi)}</div></div><div class="text-right flex-shrink-0"><div class="text-sm font-bold text-emerald-400">${ssrInterpolate(unref(fmt).rupiah(p.actual_revenue))}</div><div class="text-xs text-gray-600">dari ${ssrInterpolate(unref(fmt).rupiah(p.revenue_target))}</div></div><div class="w-20 flex-shrink-0"><div class="h-1.5 bg-navy-700 rounded overflow-hidden"><div class="h-full bg-emerald-500 rounded" style="${ssrRenderStyle(`width:${p.revenue_target ? Math.min(p.actual_revenue / p.revenue_target * 100, 100) : 0}%`)}"></div></div><div class="text-xs text-gray-600 text-right mt-0.5">${ssrInterpolate(p.revenue_target ? Math.round(p.actual_revenue / p.revenue_target * 100) : 0)}% </div></div></div>`);
        });
        ...
        if ((_b = unref(data).at_risk_projects) == null ? void 0 : _b.length) {
        ...
        _push(`</div>`);
        if ((_c = unref(data).zero_projects) == null ? void 0 : _c.length) {
...
//# sourceMappingURL=insights-C7lNAZ8j.mjs.map
