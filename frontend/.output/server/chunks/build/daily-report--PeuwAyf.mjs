  ...
  setup(__props) {
    ...
    const trendData = computed(() => {
      ...
      for (const r of reports.value) {
        ...
        map.get(d).fu += (_a = r.fu_count) != null ? _a : 0;
        map.get(d).visit += (_b = r.visit_count) != null ? _b : 0;
      }
    ...
    const salesStats = computed(() => {
      ...
      const map = /* @__PURE__ */ new Map();
      for (const r of reports.value) {
        const name = (_a = r.sales_nama) != null ? _a : "\u2014";
    ...
    const moodData = computed(() => ({
      great: reports.value.filter((r) => {
        var _a;
        return ["great", "happy", "good"].includes((_a = r.mood) != null ? _a : "");
      }).length,
      okay: reports.value.filter((r) => {
        var _a;
        return ["okay", "neutral"].includes((_a = r.mood) != null ? _a : "");
      }).length,
      tough: reports.value.filter((r) => {
        var _a;
        return ["tough", "sad", "hard"].includes((_a = r.mood) != null ? _a : "");
      }).length
    }));
    ...
    async function loadReports(resetPage = false) {
      ...
      try {
        ...
        reports.value = ((_a = data.reports) != null ? _a : []).map((r) => ({ ...r, _open: false }));
        totalCount.value = (_b = data.total) != null ? _b : 0;
        totalPages.value = (_c = data.total_pages) != null ? _c : 1;
      } catch (e) {
        errorMsg.value = (_f = (_e = (_d = e == null ? void 0 : e.data) == null ? void 0 : _d.message) != null ? _e : e == null ? void 0 : e.message) != null ? _f : String(e);
      } finally {
        loading.value = false;
      }
    }
    ...
    function moodEmoji(mood) {
      if (["great", "happy", "good"].includes(mood)) return "\u{1F60A}";
      if (["okay", "neutral"].includes(mood)) return "\u{1F610}";
      if (["tough", "sad", "hard"].includes(mood)) return "\u{1F614}";
      return "\u2014";
    }
    ...
    function statusLabel(status) {
      ...
      if (status === "reviewed") return "\u{1F441}\uFE0F Reviewed";
      return "\u270F\uFE0F Draft";
    }
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(reports).length && !unref(loading)) {
        ...
        _push(`<text x="60" y="60" text-anchor="middle" dominant-baseline="central" style="${ssrRenderStyle({ "transform": "rotate(90deg)", "transform-origin": "60px 60px" })}" class="fill-white text-sm font-bold" font-size="20" font-weight="700">${ssrInterpolate(unref(moodTotal))}</text><text x="60" y="76" text-anchor="middle" style="${ssrRenderStyle({ "transform": "rotate(90deg)", "transform-origin": "60px 60px" })}" class="fill-gray-400" font-size="9" fill="#9ca3af">laporan</text></svg></div><div class="space-y-2"><div class="flex items-center justify-between text-xs"><div class="flex items-center gap-1.5"><div class="w-2.5 h-2.5 rounded-full bg-green-500"></div><span class="text-apex-muted">Semangat \u{1F60A}</span></div><span class="font-bold text-apex-text">${ssrInterpolate(unref(moodData).great)}</span></div><div class="flex items-center justify-between text-xs"><div class="flex items-center gap-1.5"><div class="w-2.5 h-2.5 rounded-full bg-yellow-500"></div><span class="text-apex-muted">Biasa \u{1F610}</span></div><span class="font-bold text-apex-text">${ssrInterpolate(unref(moodData).okay)}</span></div><div class="flex items-center justify-between text-xs"><div class="flex items-center gap-1.5"><div class="w-2.5 h-2.5 rounded-full bg-red-500"></div><span class="text-apex-muted">Berat \u{1F614}</span></div><span class="font-bold text-apex-text">${ssrInterpolate(unref(moodData).tough)}</span></div></div></div><div class="lg:col-span-3 bg-apex-surface border border-apex-border rounded-xl p-5"><div class="text-xs text-apex-muted uppercase tracking-wider font-medium mb-4 flex items-center gap-2"><i class="fa-solid fa-users text-blue-400"></i>Kepatuhan Laporan per Sales </div><div class="space-y-3"><!--[-->`);
      ...
      if (unref(loading)) {
        ...
        _push(`<div class="mx-6 mt-5 bg-apex-surface border border-dashed border-apex-border rounded-xl p-16 text-center"><div class="text-5xl mb-4">\u{1F4CB}</div><div class="font-bold text-apex-muted text-lg mb-2">Belum ada laporan</div><div class="text-apex-faint text-sm">Laporan harian dikirim sales via APEX Mobile App.</div></div>`);
      } else {
        _push(`<div class="px-6 pt-5 pb-10"><div class="space-y-3 md:hidden"><!--[-->`);
        ssrRenderList(unref(reports), (r) => {
          var _a, _b;
          _push(`<div class="bg-apex-surface border border-apex-border rounded-xl overflow-hidden"><div class="flex items-center gap-3 px-4 py-3 cursor-pointer"><div class="${ssrRenderClass([moodBg(r.mood), "w-10 h-10 rounded-full flex items-center justify-center shrink-0 text-xl"])}">${ssrInterpolate(moodEmoji(r.mood))}</div><div class="flex-1 min-w-0"><div class="flex items-center gap-2 flex-wrap"><span class="font-semibold text-apex-text text-sm">${ssrInterpolate(r.sales_nama)}</span><span class="${ssrRenderClass([statusClass(r.status), "text-xs px-2 py-0.5 rounded-full font-medium"])}">${ssrInterpolate(statusLabel(r.status))}</span></div><div class="text-xs text-apex-muted mt-0.5"> \u{1F4C5} ${ssrInterpolate(r.report_date)} \xA0\xB7\xA0 ${ssrInterpolate((_a = r.fu_count) != null ? _a : 0)} FU \xA0\xB7\xA0 ${ssrInterpolate((_b = r.visit_count) != null ? _b : 0)} kunjungan </div></div><i class="${ssrRenderClass([r._open ? "rotate-180" : "", "fa-solid fa-chevron-down text-apex-faint text-xs transition-transform shrink-0"])}"></i></div>`);
          if (r._open) {
            ...
            if (r.notes_plan) {
              _push(`<div class="bg-blue-950/40 border border-blue-900/40 rounded-lg p-3"><div class="text-xs font-bold text-blue-400 mb-1">\u{1F4CC} Rencana Besok</div><p class="text-apex-muted text-sm leading-relaxed whitespace-pre-line">${ssrInterpolate(r.notes_plan)}</p></div>`);
            } else {
              _push(`<!---->`);
            }
        ...
        ssrRenderList(unref(reports), (r) => {
          var _a, _b, _c;
          _push(`<!--[--><tr class="hover:bg-apex-card/40 cursor-pointer transition-colors"><td class="px-5 py-3 font-mono text-apex-muted text-xs whitespace-nowrap">${ssrInterpolate(r.report_date)}</td><td class="px-4 py-3 font-semibold text-apex-text">${ssrInterpolate(r.sales_nama)}</td><td class="px-4 py-3 text-center"><span class="${ssrRenderClass([statusClass(r.status), "px-2.5 py-1 rounded-full text-xs font-semibold"])}">${ssrInterpolate(statusLabel(r.status))}</span></td><td class="px-4 py-3 text-center text-lg">${ssrInterpolate(moodEmoji(r.mood))}</td><td class="px-4 py-3 text-center font-bold text-apex-text">${ssrInterpolate((_a = r.fu_count) != null ? _a : 0)}</td><td class="px-4 py-3 text-center font-bold text-apex-text">${ssrInterpolate((_b = r.visit_count) != null ? _b : 0)}</td><td class="px-4 py-3 text-center font-bold text-apex-text">${ssrInterpolate((_c = r.new_lead_count) != null ? _c : 0)}</td><td class="px-4 py-3 text-xs text-apex-muted max-w-[180px]">`);
          if (r.notes_obstacle) {
          ...
          if (r._open) {
            ...
            if (r.notes_plan) {
              _push(`<div class="bg-blue-950/40 border border-blue-900/40 rounded-lg p-3"><div class="text-xs font-bold text-blue-400 mb-2">\u{1F4CC} Rencana Besok</div><p class="text-apex-muted text-xs leading-relaxed whitespace-pre-line">${ssrInterpolate(r.notes_plan)}</p></div>`);
            } else {
              _push(`<!---->`);
            }
            ...
            _push(`<div class="text-xs text-apex-faint"> ID: #${ssrInterpolate(r.id)} \xA0\xB7\xA0 Dibuat: ${ssrInterpolate(r.created_at ? r.created_at.slice(0, 16).replace("T", " ") : "\u2014")}</div></div></div></td></tr>`);
          } else {
            _push(`<!---->`);
          }
          _push(`<!--]-->`);
...
//# sourceMappingURL=daily-report--PeuwAyf.mjs.map
